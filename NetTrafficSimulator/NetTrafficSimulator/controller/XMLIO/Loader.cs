...
namespace NetTrafficSimulator
{
	...
	public class Loader
	{
				...
				case "network":
					...
					if (node.HasAttribute ("flush")) {
						...
						if (flush > expiry) {
							...
							throw new ArgumentException ("NetworkNode " + name + " illegal flush timer. Flush timer must be greater than expiry timer. Flush:" + flush + ", Expiry:" + expiry);
						}
					} else {
						if (6 > expiry) {
							nm.SetNetworkNodeFlushTimer (name, 6);
						} else {
							log.Warn ("Expiry timer for NetworkNode " + name + " is greater than default value for flush timer (expiry:" + expiry + ", default for flush: 6), setting flush timer to " + (expiry + 1));
							nm.SetNetworkNodeFlushTimer (name, expiry + 1);
						}
					}
		...
		public SimulationModel LoadSM(){
			...
			bool trace_random = false;
			if (ttr.HasAttribute ("trace_random")) {
				trace_random = ttr.GetAttribute("trace_random").ToLower().Equals ("true");
			}
			...
			if (events != null) {
				...
				if ((nl != null) && (ttr != null)) {
					...
					sm = new SimulationModel (trace_random);
